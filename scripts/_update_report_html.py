...
# 归因分析模板
narratives = [
    '该簇中指标间的依赖和传导关系形成紧密的业务闭环。DEPENDS_ON链向下游传导影响，TRANSMISSION路径在横向维度上扩散波动。重点关注中心节点的趋势变化。',
    '该簇展示了跨层级的归因传导网络。中心指标的波动通过多条DEPENDS_ON和TRANSMISSION链路同时向上下游扩散，形成多路径影响传导。',
    '该簇中指标关系以依赖驱动为主，DEPENDS_ON链表明下层指标变化直接受上层指标制约。TRANSMISSION路径揭示了同层级指标间的横向联动效应。',
    '该簇揭示了资源配置与业务产出之间的传导路径。AGGREGATES聚合关系将分散指标归集到核心度量，DEPENDS_ON链反映逐级驱动的约束关系。',
    '该簇中因果驱动关系（CAUSES）与数据传导（TRANSMISSION）交织，形成复杂的业务影响网络。中心指标异常会通过多重路径影响多个下游指标。',
    '该簇展示了业务执行进度与财务结果的联动链，形成投入到过程到结果的完整链路，建议关注关键节点的波动传导。',
    '该簇中多个指标围绕同一业务主题形成紧密关联，通过DEPENDS_ON和TRANSMISSION关系构成闭环反馈，建议从拓扑角度分析枢纽节点。',
    '该簇中关系类型以TRANSMISSION为主，表明数据在不同维度间的流动和转化，中心节点波动沿传导链逐级传递到末端，形成涟漪效应。',
]
